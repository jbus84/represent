...
def demo_ternary_ctl():
    ...
    try:
        generator = TernaryCTLGenerator(
            marginal_change_thres=0.02, window_size=10, target_name="ternary_ctl"
        )
        builder = ModularDatasetBuilder([generator])
...
def demo_oracle_labelling():
    ...
    try:
        generator = OracleBinaryTrendGenerator(transaction_cost=0.001, target_name="oracle_binary")
        builder = ModularDatasetBuilder([generator])
    ...
    try:
        generator = OracleTernaryTrendGenerator(
            transaction_cost=0.001, neutral_reward_factor=0.5, target_name="oracle_ternary"
        )
        builder = ModularDatasetBuilder([generator])
...
def demo_mixed_targets():
    ...
    try:
        ...
        from represent import DirectionalMFEGenerator, QuantileClassificationGenerator
