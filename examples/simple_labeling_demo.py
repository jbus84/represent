...
def create_labeling_showcase():
    ...
    generators = [
        ...
        PriceMovementGenerator(lookforward_window=50, target_name="price_movement"),
        VolatilityGenerator(window_size=30, target_name="volatility"),
    ]
    ...
    for i, (regime, color) in enumerate(zip(regimes, colors, strict=False)):
    ...
    for i in range(13):
        mask = labels_13 == i
        if np.any(mask):
            ax.scatter(timestamps[mask], labels_13[mask], c=[colors_13[i]], s=1, alpha=0.6)
    ...
    ax.plot(
        timestamps[valid_buy], mfe_buy[valid_buy], "g-", linewidth=1, alpha=0.7, label="MFE Buy"
    )
    ax.plot(
        timestamps[valid_sell], mfe_sell[valid_sell], "r-", linewidth=1, alpha=0.7, label="MFE Sell"
    )
    ...
    ax.plot(
        timestamps[valid_movement], price_movement[valid_movement], "b-", linewidth=1, alpha=0.7
    )
    ax.axhline(y=0, color="gray", linestyle="--", alpha=0.5)
    ...
    stats_text += "5-Class Distribution:\n"
    for i in range(5):
    ...
    plt.suptitle(
        "Represent Package: Modular Target Generation Showcase",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
...
def create_simple_comparison_plot(market_data, dataset):
    ...
    for i in range(5):
        ...
        if np.any(mask):
            ax.scatter(
                timestamps[mask], prices[mask], c=colors[i], s=3, alpha=0.6, label=f"Class {i}"
            )

    ax.set_title(
        "Price Series with 5-Class Quantile Classification", fontsize=14, fontweight="bold"
    )
    ax.set_ylabel("Price")
    ...
    ax.set_title(
        "Directional MFE (Maximum Favorable Excursion) Targets", fontsize=14, fontweight="bold"
    )
    ax.set_ylabel("MFE (Basis Points)")
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def main():
    ...
    try:
        create_labeling_showcase()

        print("\n🎉 DEMO COMPLETE!")
        print("📊 Created visualization plots for README")
        print("📁 Files saved in examples/")
