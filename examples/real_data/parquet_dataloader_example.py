...
def demonstrate_parquet_loading():
    ...
    if dataset._current_data is not None:
        ...
        print(
            f"   📊 Date range: {dataset._current_data['date'].min()} to {dataset._current_data['date'].max()}"
        )
    else:
...
def demonstrate_tensor_generation(dataset):
    ...
    try:
        ...
        if isinstance(batch_data, tuple):
            ...
            print(
                f"   📊 Memory usage: {tensor.numel() * tensor.element_size() / 1024 / 1024:.1f}MB"
            )
            print(
                f"   📊 Classification target shape: {classification_target.shape if classification_target is not None else 'None'}"
            )
