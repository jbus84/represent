...
def demo_single_target_generation():
    ...
    dataset = builder.build_dataset(sample_data)
    print(f"   Result columns: {dataset.columns}")
    print(f"   MFE buy range: [{dataset['mfe_buy_bps'].min():.1f}, {dataset['mfe_buy_bps'].max():.1f}] BPS")
...
def demo_multi_target_generation():
    ...
    print(f"   Target columns: {[col for col in dataset.columns if col not in sample_data.columns]}")
