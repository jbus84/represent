...
from represent.constants import (
    SAMPLES, PRICE_LEVELS, TIME_BINS, DEFAULT_FEATURES, ASK_COUNT_COLUMNS, BID_COUNT_COLUMNS, VARIANCE_COLUMN
)
...
class TestExtendedFeatures:
    ...
    def test_single_feature_variance(self):
        ...
        data = data.with_columns(
            pl.lit(0.1).alias(VARIANCE_COLUMN)
        )
    ...
    def test_invalid_features(self):
        ...
        with pytest.raises(ValueError, match="Too many features"):
            MarketDepthProcessor(features=['volume', 'variance', 'trade_counts', 'volume'])  # 4 features
...
class TestFeaturePerformance:
    ...
    def test_single_vs_multiple_feature_performance(self):
        ...
        assert multi_time < single_time * 3.0, f"Multi-feature too slow: {multi_time:.4f}s vs {single_time:.4f}s"
