...
class TestCurrencyConfigurations:
    ...
    def test_audusd_config(self):
        ...
        assert config.classification.lookforward_input == 5000

    def test_gbpusd_config(self):
        """Test GBPUSD configuration."""
        config = load_currency_config("GBPUSD")

        assert config.currency_pair == "GBPUSD"
        assert config.classification is not None

    def test_eurjpy_config(self):
        """Test EURJPY configuration."""
        config = load_currency_config("EURJPY")

        assert config.currency_pair == "EURJPY"
        assert config.classification is not None
