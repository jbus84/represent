...
class TestDataStructuresCoverage:
    ...
    def test_price_lookup_table_operations(self):
        ...
        # Test valid price lookups (use bid price, which is mid_price - 1)
        valid_bid_price = int(mid_price) - 1
        result = lookup.price_to_index(valid_bid_price)
        assert result >= 0
        ...
        # Test vectorized lookup (use valid bid and ask prices, not mid price)
        prices = np.array([int(mid_price) - 1, int(mid_price) + 1, int(mid_price) - 2])
