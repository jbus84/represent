...
def create_time_bins(df: pl.DataFrame, samples: int = SAMPLES) -> pl.DataFrame:
    ...
    return df.with_columns((pl.int_range(0, samples) // TICKS_PER_BIN).alias("tick_bin"))
...
def create_price_bins(
    mid_price: float, price_range: int = 200
...
def process_side_data(
    df: pl.DataFrame, price_columns: list, volume_columns: list, price_to_index: Dict[int, int]
) -> np.ndarray:
    """Process ask or bid side data into market volume array."""
    # Group by time bins and calculate means/medians
    grouped_prices = (
        df[price_columns + ["tick_bin"]].group_by(["tick_bin"]).mean().sort(by="tick_bin")
    )
    grouped_prices = grouped_prices.with_columns(
        [pl.col(col) // 1 for col in price_columns]
    )  # Floor division

    grouped_volumes = (
        df[volume_columns + ["tick_bin"]]
        .group_by(["tick_bin"])
        .median()
        .sort(by="tick_bin")[volume_columns]
    )
...
def create_normed_abs_combined(
    ask_market_volume: np.ndarray, bid_market_volume: np.ndarray
) -> np.ndarray:
