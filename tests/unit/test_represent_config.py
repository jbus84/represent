...
class TestRepresentConfig:
    ...
    def test_computed_fields(self):
        ...
        config = RepresentConfig(samples=25000, ticks_per_bin=50)

        # time_bins should be computed as samples // ticks_per_bin
        assert config.time_bins == 25000 // 50  # = 500
        ...
        config_custom = RepresentConfig(samples=30000, ticks_per_bin=50, min_symbol_samples=400)
...
class TestCreateRepresentConfig:
    ...
    def test_create_custom_config(self):
        ...
        config = create_represent_config(
            currency="GBPUSD",
            nbins=9,
            samples=30000,  # Above minimum threshold
            features=["volume", "variance"],
            lookback_rows=2000,
        )
