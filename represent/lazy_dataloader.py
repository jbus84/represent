...
class LazyParquetDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"📊 Lazy dataset initialized: {len(self.indices):,} samples from {self.parquet_path.name}")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Load sample lazily
        sample_data = self._lazy_frame.slice(actual_idx, 1).collect()
        ...
        if self.shape_column and self.shape_column in sample_data.columns:
            ...
            shape = eval(shape_str)  # Parse shape string like "(402, 500)"
        else:
        ...
        if isinstance(features_data, str):
            # Hex-encoded string - convert back to bytes
        ...
        features_tensor = torch.from_numpy(
            np.frombuffer(features_bytes, dtype=np.float32).reshape(shape)
        )
        
        # Get classification label
        label_tensor = torch.tensor(
            sample_data[self.labels_column][0], 
            dtype=torch.long
        )
    ...
    def get_metadata(self, idx: int) -> dict:
        ...
        metadata_cols = [
            'sample_id', 'start_timestamp', 'end_timestamp', 'target_timestamp',
            'symbol', 'source_file', 'date', 'global_start_idx', 'global_end_idx'
        ]
    ...
    def get_dataset_info(self) -> dict:
        ...
        if self.labels_column in available_columns:
            query_exprs.append(pl.col(self.labels_column).value_counts().alias('label_distribution'))
