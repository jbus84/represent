...
from .constants import (
    ...
    ASK_COUNT_COLUMNS, BID_COUNT_COLUMNS, ASK_ANCHOR_COLUMN, BID_ANCHOR_COLUMN, PRICE_LEVELS, TIME_BINS,
    VOLUME_DTYPE, FEATURE_TYPES, DEFAULT_FEATURES, FEATURE_INDEX_MAP, MAX_FEATURES, 
    FeatureType, get_output_shape
)
...
class MarketDepthProcessor:
    ...
    def __init__(self, features: Optional[Union[list[str], list[FeatureType]]] = None):
        ...
        if invalid_features:
            raise ValueError(f"Invalid features: {invalid_features}. Valid options: {FEATURE_TYPES}")
        ...
        for feature in self.features:
            self._temp_ask_volumes[feature] = np.empty((PRICE_LEVELS, TIME_BINS), dtype=VOLUME_DTYPE)
            self._temp_bid_volumes[feature] = np.empty((PRICE_LEVELS, TIME_BINS), dtype=VOLUME_DTYPE)
    ...
    def process(self, df: pl.DataFrame) -> np.ndarray:
        ...
        if len(self.features) == 1:
            ...
            result = self._output_buffers[feature].compute_normalized_difference(ask_cumulative, bid_cumulative)
            return result.copy()
        else:
            ...
            for feature in self.features:
                ask_cumulative = self._ask_grids[feature].get_cumulative_volume(reverse=False)
                bid_cumulative = self._bid_grids[feature].get_cumulative_volume(reverse=True)
                feature_result = self._output_buffers[feature].compute_normalized_difference(ask_cumulative, bid_cumulative)
...
def create_processor(features: Optional[Union[list[str], list[FeatureType]]] = None) -> MarketDepthProcessor:
...
def process_market_data(df: pl.DataFrame, features: Optional[Union[list[str], list[FeatureType]]] = None) -> np.ndarray:
    """
