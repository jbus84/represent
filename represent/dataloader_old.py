...
class MarketDepthDataset(IterableDataset[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    def _initialize_data_source(self) -> None:
        ...
        if isinstance(self._data_source, (str, Path)):
            ...
            if self.use_memory_mapping and path.suffix.lower() in {'.dbn', '.zst', '.parquet'}:
    ...
    def _setup_memory_mapping(self, path: Path) -> None:
        ...
        if path.suffix.lower() == '.parquet':
            # Use polars for efficient parquet loading with memory optimization
            self._current_data = pl.read_parquet(str(path))
        else:
            # Handle DBN files using databento
    ...
    def _load_file_data(self, path: Path) -> None:
        ...
        if path.suffix.lower() == '.parquet':
            # Load parquet file directly with polars for optimal performance
            self._current_data = pl.read_parquet(str(path))
        else:
            # Handle DBN files using databento
            import databento as db
